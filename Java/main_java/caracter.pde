...
class Caracter {
  ...
  void update() {
    boolean onScreen = x>Stage.pos-2*OFFSET && x<Stage.pos+width+2*OFFSET;
    if (type == PLAYER || onScreen) {
      memory = countdown(memory);
      if ( memory > 0) {println (memory); println(lives);}
  ...
  void displayManager() {
    int index = 0;
    if (lives == 0) {
      index = 4; s = sprite(fDead, fDelay);
      memory = 6000;
    }
    else if (attack) {
      if (direction > 0) {
        index = 4; s = sprite(fAtk, fDelay); 
      }
      else {
        index = 5; s = sprite(fAtk, fDelay);
      }
  }
